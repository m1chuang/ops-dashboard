...
(function(factory) {
	...
	var DraggableGridList = function(element, options, draggableOptions) {
		...
		this.draggableOptions = $.extend({}, this.draggableDefaults,
			draggableOptions);
	...
	DraggableGridList.prototype = {
		...
		_generateItemsFromDOM: function() {
			...
			this.$items.each(function(i, element) {
				...
				var right_position = Number($(element).attr('data-x'))+Number($(element).attr('data-w'));
				next_empty=(next_empty > right_position)?next_empty:right_position;
			});
			return items;
		},
		...
		_snapItemPositionToGrid: function(item) {
			...
			console.log([col, row]);
			console.log(item.$element.data('y'));
			item.$element.data('x', col);
			item.$element.data('y', row);
			console.log(item.$element.data('y'))
			return [col, row];
		},
