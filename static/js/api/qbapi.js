...
aims.query({
	...
	field_raw: "{'5'.IR.'last+3+mon'}AND{'126'.CT.'ICS'}AND({'10'.CT.'Registry'}OR{'10'.CT.'Messaging'}OR{'10'.CT.'API Portal'}OR{'10'.CT.'API Gateway'})",
	extra: [
		"&clist=126.5.10.8"
	],
			...
			function(i, val) {
				...
				if (service_name in incidents) {
					...
					incidents[service_name]['uptime'] = (1 - incidents[
						service_name]['total_duration'] / (3 * 30 * 24 * 60)).toPrecision(4);

				} else {
					incidents[service_name] = {
						count: 1,
						total_duration: incident_duration,
						uptime: (1 - incident_duration / (3 * 30 * 24 * 60)).toPrecision(4)
					}
				};
